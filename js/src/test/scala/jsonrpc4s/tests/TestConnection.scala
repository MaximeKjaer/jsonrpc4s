...
object TestConnection {
  ...
  def apply(
    ...
    val inAlice = new ByteArrayInputStream(Array.emptyByteArray)
    val inBob = new ByteArrayInputStream(Array.emptyByteArray)
    val outAlice = new ByteArrayOutputStream() // TODO connect to inBob
    val outBob = new ByteArrayOutputStream() // TODO connect to inAlice
